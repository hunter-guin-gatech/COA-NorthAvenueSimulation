...
def result():
    ...
    tifCounter = len(glob.glob1("//ad.gatech.edu/gtfs/COE/CEE/Transpo/smartdata/RealTimeRunTrajectoryFiles_5Sep2019/trajectoryfiles","*.json"))
    filename = "//ad.gatech.edu/gtfs/COE/CEE/Transpo/smartdata/RealTimeRunTrajectoryFiles_5Sep2019/trajectoryfiles/trajectoryfile_"+d+"_"+t+"_"+str(tifCounter)+".json"

    with open(filename, 'w') as outfile:
    	json.dump(request.json, outfile)
    	#json.dumps(str(request.json).replace('"', ''))
    return str(request.json).replace('"', '')
    #requests.delete(timeout=2.50)
